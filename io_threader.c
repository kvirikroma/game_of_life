...
void* input_thread_function(void* parameters)
{
    ...
    while (!(const volatile bool)self->stop_flag)
    {
        if (*self->lmb_pressed ^ *self->rmb_pressed)
        {
            ...
            life_drawer_change_cell((life_drawer*)&self->drawer, x, y, *self->lmb_pressed);
            if (*self->move)
            {
...
{
    io_threader* self = (io_threader*)parameters;
    life_drawer_init((life_drawer*)&self->drawer, self->window_x, self->window_y, self->cells_x, self->cells_y);
    ...
    while (!(const volatile bool)self->stop_flag)
    {
        ...
        if (self->stop_flag)
        {
            ...
            life_drawer_delete((life_drawer*)&self->drawer);
            return 0;
        }
        life_drawer_redraw((life_drawer*)&self->drawer);
